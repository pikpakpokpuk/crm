...
function NewEmployeeModal({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  const [form, setForm] = useState<NewEmployeeForm>(EMPTY_NEW);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await api.post('/employees', { name: form.name.trim(), email: form.email.trim(), password: form.password, role: form.role });
      onSaved(); onClose();
    } catch (err: unknown) {
...
function EditEmployeeModal({ employee, onClose, onSaved }: { employee: Employee; onClose: () => void; onSaved: () => void }) {
  ...
  const set = (f: keyof EditEmployeeForm) => (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) =>
    setForm((p) => ({ ...p, [f]: e.target.value }));
...
export default function EmployeesPage() {
  ...
  const [modal, setModal] = useState<{ type: 'new' } | { type: 'edit'; employee: Employee } | null>(null);
  ...
  return (
    <div className="p-6 space-y-5">
      {modal?.type === 'new' && <NewEmployeeModal onClose={() => setModal(null)} onSaved={loadEmployees} />}
      {modal?.type === 'edit' && <EditEmployeeModal employee={modal.employee} onClose={() => setModal(null)} onSaved={loadEmployees} />}
          ...
          {employees.map((emp) => (
            <div key={emp.id} className="card p-5">
              ...
              <div className="mt-4 flex gap-2">
                <button className="btn btn-secondary text-xs flex-1 justify-center" onClick={() => setModal({ type: 'edit', employee: emp })}>Edit</button>
              </div>
            </div>
          ))}
          {employees.length === 0 && (
